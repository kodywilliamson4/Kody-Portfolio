...
// DO 2: Fill in this part by updating EACH component of the metrics structure appropriately.
    metrics.tot_comps = metrics.tot_comps + count;
    metrics.tot_calls = metrics.tot_calls + 1;
    if (metrics.max_comps < count)
    {
        metrics.max_comps = count;
    }
    if (metrics.min_comps > count)
    {
...
// DO 3: Fill in this part by reporting the components of the metrics structure as indicated in the sample output.
        cout << "Minimum number of comparisons = " << metrics.min_comps << endl;
        cout << "Maximum number of comparisons = " << metrics.max_comps << endl;
        cout << "Average number of comparisons = " << metrics.tot_comps / metrics.tot_calls << endl;
        cout << "Total number of times searched = " << metrics.tot_calls << endl;
    }
}
