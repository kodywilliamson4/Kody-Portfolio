...
void PSO(int Np, int Nd, double (*objFunc)(std::vector<double> &, int, int))
{
    ...
    std::cout << gBestValue << "," << Np << "," << Nd <<"," << numEvals <<"," << timer.getElapsedTime() << std::endl;
}
...
int main(){
    ...
    {
        for(int j = NpMin; j <= NpMax; j = j + NpStep)
        {
            Np = j;
            PSO(Np, Nd, rPtr);
        }
